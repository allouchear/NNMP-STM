...
def validation_test(trainer, lossbest, best_checkpoint, best_loss_file, step):
	...
	#means,loss=trainer.computeTrainAccuracy(verbose=False)
	print_means(means)
	st='Loss={:0.8}'.format(loss.numpy())
	if lossbest is not None:
		st = st +'\t ; Old best loss={:0.8}'.format(lossbest.numpy())
	print(st)
...
def create_trainer(stmModel, args):
	print("Creation of trainer")
	trainer=Trainer(stmModel, args)
	return  trainer
...
def add_train_metrics_to_files_from_sums(trainer, train_metrics_files, bypixel, sums, addTitle=False):
	acc=trainer.computeAccuraciesFromSums(sums, bypixel, verbose=False)
